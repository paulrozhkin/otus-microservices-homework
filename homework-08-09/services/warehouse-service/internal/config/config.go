...
func Load() (Config, error) {
	cfg, err := platformconfig.NewBuilder[Config]().WithAppName("warehouse-service").WithConfigPath("./services/warehouse-service/config").WithEnvPrefix("OTUS_WAREHOUSE_SERVICE").
		...
		WithDefaultValues(map[string]any{"http.addr": ":8004", "kafka.brokers": "kafka:29092", "kafka.topic": contracts.TopicWarehouseCommands, "kafka.groupId": "warehouse-service-v1", "outbox.pollInterval": 500 * time.Millisecond}).Load()
	if err != nil {
		return Config{}, err
	}
	return *cfg, nil
