...
func NewOrderRepository(db *gorm.DB) OrderRepository { return &OrderRepositoryImpl{db: db} }
...
func orderToDAO(order *entity.Order) *Order {
	return &Order{ID: order.ID, UserID: order.UserID, Email: order.Email, Price: order.Price, Status: string(order.Status), FailureReason: order.FailureReason, CreatedAt: order.CreatedAt, UpdatedAt: order.UpdatedAt}
}

func orderFromDAO(dao *Order) *entity.Order {
	return &entity.Order{ID: dao.ID, UserID: dao.UserID, Email: dao.Email, Price: dao.Price, Status: entity.Status(dao.Status), FailureReason: dao.FailureReason, CreatedAt: dao.CreatedAt, UpdatedAt: dao.UpdatedAt}
}
